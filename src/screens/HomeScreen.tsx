...
import { TrailCategory } from "../types";
...
export const HomeScreen = () => {
  ...
  const featured = visibleTrails.slice(0, 3);
  const trending = visibleTrails.slice(3);
    ...
    <div className="w-full h-full bg-[#F0E8DC] text-[#2B4A35] select-none overflow-y-auto pb-24">
      ...
      <div className="px-6 pt-5 pb-3 flex justify-between items-center bg-transparent">
        <div className="flex items-center gap-3">
          ...
          />
          <div>
            <p className="text-[11px] text-[#2B4A35]/65 leading-none mb-0.5">
              Good morning
            </p>
        ...
        <div className="flex items-start gap-3">
          <div className="flex-1">
            <div className="flex items-center gap-1.5">
              ...
              <span className="px-1.5 py-0.5 rounded-sm bg-amber-500/10 text-[#D4A084] text-[6px] font-bold tracking-wide uppercase border border-[#D4A084]/20">
                Cognitive AI
              </span>
            </div>
            ...
            <p className="font-sans text-xs text-neutral-200/90 leading-relaxed font-light mt-1 max-w-[220px]">
              Feeling drained, foggy, or anxious? Match your cognitive state
              with nature's restorative dimensions.
            </p>
      ...
      {/* Featured Trail Cards — horizontal editorial cards (handoff dimensions:
          270px wide, 170px photo on top, white info block below) */}
      <div className="flex gap-3.5 overflow-x-auto px-6 py-2 scrollbar-none snap-x snap-mandatory">
        {featured.map((trail) => (
          <div
            key={trail.id}
            onClick={() => navigate(`/trail/${trail.id}`)}
            className="w-[270px] shrink-0 snap-start bg-white rounded-[22px] overflow-hidden shadow-xs hover:shadow-md cursor-pointer transition-shadow duration-300"
          >
            <div className="relative">
              ...
              <button
                ...
                className="absolute top-3 right-3 w-[34px] h-[34px] rounded-full bg-white/90 backdrop-blur-sm flex items-center justify-center text-[#2B4A35] hover:bg-white transition-colors shadow-sm"
              >
                <Heart
                  size={16}
                  fill={liked[trail.id] ? "#2B4A35" : "transparent"}
                />
              </button>
              <div className="absolute bottom-2.5 left-2.5 px-2.5 py-1 rounded-full bg-[#2B4A35]/75 backdrop-blur-sm text-white text-[10px] font-bold uppercase tracking-wide">
                {trail.difficulty}
              </div>
            </div>

            <div className="p-3.5 pt-3">
              <h4 className="font-sans text-[15px] font-bold text-[#2B4A35] truncate">
                {trail.name}
              </h4>
              <p className="text-[11px] text-[#2B4A35]/60 mt-0.5 truncate">
                {trail.location}
              </p>
              <div className="flex items-center justify-between mt-2 gap-2">
                <div className="flex items-center gap-1 text-[#D4A084] shrink-0">
                  <Star size={12} fill="currentColor" />
                  <span className="text-xs font-bold text-[#2B4A35] leading-none">
                    {trail.rating}
                  </span>
                </div>
                <span className="text-[11px] text-[#2B4A35]/60 font-medium truncate">
      ...
      {trending.length > 0 && (
        <>
          <div className="px-6 mt-4 mb-2 flex justify-between items-center">
            <h3 className="font-serif text-xl font-normal tracking-wide text-[#2B4A35]">
              Trending nearby
            </h3>
            <Link
              to="/map"
              className="text-xs font-semibold text-[#D4A084] hover:underline"
            >
              See all
            </Link>
          </div>
          ...
          <div className="px-6 flex flex-col gap-3">
            {trending.map((trail) => (
              ...
              >
                ...
                />
                <div className="flex-1">
                  <h4 className="font-sans text-sm font-bold text-[#2B4A35]">
                    {trail.name}
                  </h4>
                  <p className="text-xs text-[#2B4A35]/65">
                    {trail.distance} · {trail.difficulty} · {trail.matchScore}%
                    Match
                  </p>
                  <div className="flex items-center gap-1 text-[#D4A084] mt-0.5">
                    ...
                    {trail.isRestorative && (
                      <span className="ml-1 px-1.5 py-0.2 rounded-sm bg-[#EBF5EE] text-[#1E3A27] text-[8px] font-bold">
                        RESTORATIVE
                      </span>
                    )}
                  </div>
